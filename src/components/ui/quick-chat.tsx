...
export function QuickChat({ sessionId: externalSessionId, onSessionChange, className = "" }: QuickChatProps) {
  ...
  const sendMessage = async (content: string, _files?: any) => {
    ...
    if (!activeSessionId) {
      try {
        const newSess = await api.createSession("محادثة سريعة");
      ...
      {showEmptyState ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-6">
          ...
          <div className="w-full max-w-2xl px-4">
            <ChatInput onSendMessage={sendMessage} disabled={isLoading} placeholder="اسأل أي شيء..." onOpenSettings={() => setShowSettings(true)} />
          </div>
        </div>
      ) : (
        <>
